...
export async function adjustBankBalance({ amount, mode = "set", reason, userId, client = prisma }) {
  ...
  return client.$transaction(async (transactionClient) => {
    ...
    const todayRpTotal = toNumber(
      (
        await transactionClient.bankOrder.aggregate({
          where: { country: 1, date: { gte: startOfDay(new Date()), lte: endOfDay(new Date()) } },
          _sum: { depositAmount: true }
        })
      )._sum.depositAmount
    );
    const currentAvailableBalance = previousManualBalance - todayRpTotal;

    const nextManualBalance = mode === "topup" ? currentAvailableBalance + normalizedAmount : normalizedAmount;
    ...
    return transactionClient.bankBalanceTransaction.create({
      data: {
        balanceId: 1,
        type: mode === "topup" ? "topup" : "opening",
        description: reason?.trim() || (mode === "topup" ? "Balance top-up" : "Daily bank balance entry"),
