...
function hasGrantedAliases(result, aliases) {
  return aliases.every((alias) => {
    const directState = normalizePermissionState(result?.[alias]);
    if (directState === "granted") return true;

    const androidPermissionState = normalizePermissionState(result?.[`android.permission.${alias}`]);
    if (androidPermissionState === "granted") return true;
...
async function requestBluetoothPermissions(methodName, requiredAliases, fallbackAliases = []) {
  if (!canUseNativePrinting() || !isBluetoothPrinterAvailable()) {
    return { granted: false, error: "BluetoothPrinter plugin is not available in this build." };
  }
  try {
    const result = await BluetoothPrinter[methodName]();
    let granted = isPermissionGrantedByMethod(methodName, result, requiredAliases, fallbackAliases);

    // Fallback to a full plugin permission request when alias-level state is inconsistent.
    if (!granted && typeof BluetoothPrinter.requestPermissions === "function") {
      const fallbackResult = await BluetoothPrinter.requestPermissions();
      const mergedResult = { ...result, ...fallbackResult };
      granted = isPermissionGrantedByMethod(methodName, mergedResult, requiredAliases, fallbackAliases);
      return { granted, result: mergedResult };
    }
...
export async function requestBluetoothConnectPermissions() {
  return requestBluetoothPermissions("requestConnectPermissions", ["bluetoothConnect"], ["bluetoothLegacy"]);
}
