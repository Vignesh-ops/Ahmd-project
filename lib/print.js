...
import { formatCurrencyPlain, formatDate, formatPreciseNumber } from "./utils";
...
export function buildBankReceiptText(order) {
  ...
  return [
    ...
    `Rate: ${formatPreciseNumber(order.rate, 5)}`,
    `Service: ${formatPreciseNumber(order.serviceCharge, 5)}`,
    "------------------------------",
    "DEPOSIT AMOUNT",
    depositAmount,
    "PAYABLE (RM)",
...
function hasGrantedAliases(result, aliases) {
  return aliases.every((alias) => result?.[alias] === "granted");
}
