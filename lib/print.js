...
export async function setPreferredPrinter(device) {
  // Ensure this function only runs in a client-side environment with localStorage
  if (typeof window === "undefined" || !window.localStorage) {
    console.warn("localStorage is not available. Cannot set preferred printer.");
    return;
  }

  // Validate the device object
  if (!device || !device.id) {
    console.warn("Invalid device provided to setPreferredPrinter. Clearing any existing preference.");
    localStorage.removeItem(LOCAL_STORAGE_PREFERRED_PRINTER_KEY); // Clear invalid or unset preference
    return;
  }

  // Save the device's ID and name to localStorage
  const printerInfo = { id: device.id, name: device.name || "Unknown Printer" };
  localStorage.setItem(LOCAL_STORAGE_PREFERRED_PRINTER_KEY, JSON.stringify(printerInfo));
}
