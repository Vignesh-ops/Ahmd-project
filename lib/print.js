...
import { formatDisplayOrderNo } from "./orderNoDisplay";
import { formatCurrencyPlain, formatDate } from "./utils";

// Lazy-load plugins to avoid instantiation errors in non-native environments
let BluetoothPrinter;
let WebPrint;

function getBluetoothPrinter() {
  if (!BluetoothPrinter) {
    BluetoothPrinter = registerPlugin("BluetoothPrinter");
  }
  return BluetoothPrinter;
}

function getWebPrint() {
  if (!WebPrint) {
    WebPrint = registerPlugin("WebPrint");
  }
  return WebPrint;
}
...
export async function printCurrentPage(options = {}) {
  if (canUseNativePrinting() && Capacitor.isPluginAvailable("WebPrint")) {
    await getWebPrint().printCurrentPage({
...
async function requestBluetoothPermissions(methodName, requiredAliases) {
  ...
  try {
    const result = await getBluetoothPrinter()[methodName]();
...
export async function openBluetoothPermissionSettings() {
  ...
  if (typeof getBluetoothPrinter().openAppSettings !== "function") {
    throw new Error("Bluetooth settings are not available in this build.");
  }
  await getBluetoothPrinter().openAppSettings();
}
...
export async function getPairedBluetoothPrinters() {
  ...
  const result = await getBluetoothPrinter().getPairedDevices();
...
export async function getAvailablePrinters() {
  ...
  try {
    bluetooth = await getPairedBluetoothPrinters();
  } catch (e) {
    console.warn("Bluetooth fetch failed:", e.message);
  }

  const [usb, preferred] = await Promise.all([
    getUsbPrinters(),
    getPreferredPrinter()
  ]);
...
export async function startBluetoothDiscovery({ onDeviceFound, onFinished } = {}) {
  ...
  const handles = [];
  if (onDeviceFound) {
    handles.push(await getBluetoothPrinter().addListener("deviceFound", onDeviceFound));
  }
  if (onFinished) {
    handles.push(await getBluetoothPrinter().addListener("discoveryFinished", onFinished));
  }

  await getBluetoothPrinter().startDiscovery();

  return {
    started: true,
    remove: async () => {
      await getBluetoothPrinter().stopDiscovery();
...
export async function pairBluetoothPrinter(address) {
  ...
  return getBluetoothPrinter().pair({ address });
}
...
export async function printReceipt(receiptText, preferredOverride = null) {
  ...
  if (preferred.type === "bluetooth") {
    ...
    await getBluetoothPrinter().print({ address: preferred.id, data });
    return { fallback: false, deviceName: preferred.name };
  }
