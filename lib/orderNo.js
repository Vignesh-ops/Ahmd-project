...
export async function generateCurrencyOrderNo(type, storeCode, prismaClient, currency = "IDR", date = new Date()) {
  ...
  // Fast counter extraction:
  // orderNo suffix is always 4 digits (padStart(4)), so avoid regexp parsing on every row.
  // This keeps the scan bounded by the LIKE prefix and removes regexp_match CPU overhead.
  const rows =
    safeType === "B"
      ? await prismaClient.$queryRaw`
          SELECT COALESCE(
            MAX(CAST(substring("orderNo" from '([0-9]{4})$') AS int)),
            0
          ) AS "lastCounter"
          FROM "BankOrder"
