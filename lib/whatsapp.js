...
async function nativeWhatsAppShare(message) {
  const [{ AppLauncher }, { Share }, { Browser }] = await Promise.all([
  ...
  try {
    ...
    const canOpenHttps = await AppLauncher.canOpenUrl({ url: waUrl });

    if (canOpenHttps.value) {
      await AppLauncher.openUrl({ url: waUrl });
      return { opened: true, returned: true, appHidden: true };
    }
  } catch {
    // Fall through to share dialog.
  }
