...
export async function getOrderSummary({ sessionUser, filters = {}, client = prisma }) {
  const where = buildBankWhere(sessionUser, filters);
  const [groupedTotals, myrPayableTotal, missingPayableOrders] = await Promise.all([
  ...
  const fallbackPayableTotal = missingPayableOrders.reduce((sum, order) => {
    return (
      sum +
      calculateTotalPayable({
        depositAmount: order.depositAmount,
  ...
  return {
    ...
    totalPayableMYR: Number(myrPayableTotal._sum.totalPayableAmount || 0) + fallbackPayableTotal
  };
}
