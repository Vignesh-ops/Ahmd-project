...
export async function getOrderSummary({ sessionUser, filters = {}, includeProfit = true, client = prisma }) {
  const where = buildBankWhere(sessionUser, filters);
  const [groupedTotals, groupedProfitRates, missingPayableGroups] = await Promise.all([
  ...
  missingPayableGroups.forEach((order) => {
    const fallbackPayable = calculateTotalPayable({
      depositAmount: order._sum.depositAmount,
      rate: order.rate,
      serviceCharge: order._sum.serviceCharge
    });
