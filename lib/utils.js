...
// Strips a comma-formatted amount back to a plain numeric string (digits + at most one dot).
...
// Adds thousands separators to a plain numeric string for display in an input field.
...
export function isLettersAndSpacesOnly(value = "") {
  return /^[\p{L} ]+$/u.test(String(value));
}

const CURRENCY_FORMATS = {
  MYR: { locale: "ms-MY", currency: "MYR", plainPrefix: "RM", maximumFractionDigits: 2 },
  INR: { locale: "en-IN", currency: "INR", plainPrefix: "RS", maximumFractionDigits: 5 },
...
export function formatDate(value) {
  ...
  // Use toLocaleDateString for better mobile compatibility
...
export function formatDateTime(value) {
  ...
  // Use toLocaleString for better mobile compatibility
...
// "YYYY-MM-DD" for the current local date - matches the format a date input's
// value/min/max attributes expect.
export function todayDateString() {
  const date = new Date();
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
}

// A plain "YYYY-MM-DD" string (from a date input) is parsed as UTC midnight by
// `new Date(string)`, but the day/month/year the user actually picked is a local
// calendar date. Parsing the parts directly avoids a UTC/local timezone shift.
...
export function calculateProfitMYR({ serviceCharge, rate }) {
  const parsedServiceCharge = parseNumber(serviceCharge, 0);
  const parsedRate = parseNumber(rate, 0);

  if (parsedServiceCharge <= 0 || parsedRate <= 0) {
    return 0;
  }

  return parsedServiceCharge * parsedRate;
}
