...
import { ArrowDownRight, ArrowUpRight, CalendarClock, ChevronLeft, ChevronRight, Landmark, RefreshCw, Wallet, X } from "lucide-react";
...
export default function BankBalanceManager({ initialData }) {
  ...
  return (
    <>
      <section className="glass-panel overflow-hidden rounded-[36px] border border-gold/15 bg-gradient-to-br from-gold/10 via-transparent to-teal/5 p-6 shadow-xl sm:p-8">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 inline-flex items-center gap-2 text-sm text-white/50"><CalendarClock className="h-4 w-4" />Last updated {updated}</p>
          </div>
          <Button icon={Landmark} onClick={() => { setMessage(""); setOpen(true); }}>Adjust Balance</Button>
        </div>
      </section>
      ...
      {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="adjust-balance-title"><form onSubmit={saveAdjustment} className="dialog-surface w-full max-w-lg rounded-[28px] border border-gold/20 p-6 shadow-2xl sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.22em] text-gold-light/70">Daily Entry</p><h2 id="adjust-balance-title" className="mt-2 text-2xl font-semibold text-white">Enter Today’s Bank Balance</h2><p className="mt-2 text-sm text-white/50">Current available balance: <span className="font-mono text-white/80">{money(data.availableBalance)}</span></p></div><button type="button" onClick={() => setOpen(false)} className="rounded-xl p-2 text-white/50 hover:bg-white/10 hover:text-white" aria-label="Close"><X className="h-5 w-5" /></button></div><div className="mt-6 space-y-4"><div><label htmlFor="balance-amount" className="mb-2 block text-sm font-medium text-white/75">Today’s Bank Balance (IDR)</label><input id="balance-amount" required min="0" step="0.01" type="number" value={form.amount} onChange={(event) => setForm((current) => ({ ...current, amount: event.target.value }))} placeholder="50,000" className="w-full rounded-2xl border border-white/10 bg-dark-input px-4 py-3 text-sm text-white outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20" /></div>{message && <p className="rounded-xl border border-red-400/20 bg-red-400/10 px-3 py-2 text-sm text-red-300">{message}</p>}</div><div className="mt-6 flex justify-end gap-3"><Button type="button" variant="secondary" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? <RefreshCw className="h-4 w-4 animate-spin" /> : null}{saving ? "Saving..." : "Save Balance"}</Button></div></form></div>}
    </>
  );
}
